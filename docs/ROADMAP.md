...
| ✅ | **3.2 Décompilation vers Yul / Pseudo-Code** | Création d'un AST (Abstract Syntax Tree) à partir du CFG, implémentation d'un émulateur de pile local, séparation des blocs par fonctions, et génération d'un code source pseudo-Solidity/Yul lisible (commandes `ast`, `yul` et `decompile`). |
