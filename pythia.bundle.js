(() => {
  ...
  var require_state = __commonJS({
    "src/state.js"(exports, module) {
      ...
      var SymbolicState = class _SymbolicState {
        constructor(z3) {
          this.z3 = z3;
          this.pc = 0;
          this.depth = 0;
          ...
          this.pathVisited = /* @__PURE__ */ new Map();
          this.txIndex = 0;
          this.txHistory = [];
          this.memory = /* @__PURE__ */ new Map();
          this.storage = /* @__PURE__ */ new Map();
          this.tstorage = /* @__PURE__ */ new Map();
        }
        /**
         * Crée une copie indépendante (fork) de cet état pour une nouvelle branche (JUMPI).
         */
        clone() {
          const newState = new _SymbolicState(this.z3);
          newState.pc = this.pc;
  ...
  var require_opcodes = __commonJS({
    "src/opcodes.js"(exports, module) {
      function getMathOpcodes(z3) {
        const Z3_ONE = z3.BitVec.val(1, 256);
        const Z3_ZERO = z3.BitVec.val(0, 256);
        const stableSymbol = (name, ...operands) => z3.BitVec.const(`${name}_${operands.map((o) => o.ast).join("_")}`, 256);
        return {
          // ── Arithmétique ───────────────────────────────────────────────────────
          1: { name: "ADD", args: 2, exec: (a, b) => a.add(b) },
          2: { name: "MUL", args: 2, exec: (a, b) => a.mul(b) },
          3: { name: "SUB", args: 2, exec: (a, b) => a.sub(b) },
          4: { name: "DIV", args: 2, exec: (a, b) => z3.If(b.eq(Z3_ZERO), Z3_ZERO, a.udiv(b)) },
          5: { name: "SDIV", args: 2, exec: (a, b) => z3.If(b.eq(Z3_ZERO), Z3_ZERO, a.sdiv(b)) },
          6: { name: "MOD", args: 2, exec: (a, b) => z3.If(b.eq(Z3_ZERO), Z3_ZERO, a.urem(b)) },
          7: { name: "SMOD", args: 2, exec: (a, b) => z3.If(b.eq(Z3_ZERO), Z3_ZERO, a.srem(b)) },
          8: { name: "ADDMOD", args: 3, exec: (a, b, m) => z3.If(m.eq(Z3_ZERO), Z3_ZERO, a.add(b).urem(m)) },
          9: { name: "MULMOD", args: 3, exec: (a, b, m) => z3.If(m.eq(Z3_ZERO), Z3_ZERO, a.mul(b).urem(m)) },
          ...
          10: { name: "EXP", args: 2, exec: (base, exp) => stableSymbol("exp", base, exp) },
          // SIGNEXTEND (0x0b): extension de signe sur b bits.
          // Complexe à encoder proprement en Z3 BitVec 256-bit ; stable UF.
          11: { name: "SIGNEXTEND", args: 2, exec: (b, x) => stableSymbol("signext", b, x) },
          // ── Comparaisons ───────────────────────────────────────────────────────
          16: { name: "LT", args: 2, exec: (a, b) => z3.If(a.ult(b), Z3_ONE, Z3_ZERO) },
          17: { name: "GT", args: 2, exec: (a, b) => z3.If(a.ugt(b), Z3_ONE, Z3_ZERO) },
          18: { name: "SLT", args: 2, exec: (a, b) => z3.If(a.slt(b), Z3_ONE, Z3_ZERO) },
          19: { name: "SGT", args: 2, exec: (a, b) => z3.If(a.sgt(b), Z3_ONE, Z3_ZERO) },
          20: { name: "EQ", args: 2, exec: (a, b) => z3.If(a.eq(b), Z3_ONE, Z3_ZERO) },
          21: { name: "ISZERO", args: 1, exec: (a) => z3.If(a.eq(Z3_ZERO), Z3_ONE, Z3_ZERO) },
          // ── Opérations binaires (Bitwise) ──────────────────────────────────────
          22: { name: "AND", args: 2, exec: (a, b) => a.and(b) },
          23: { name: "OR", args: 2, exec: (a, b) => a.or(b) },
          24: { name: "XOR", args: 2, exec: (a, b) => a.xor(b) },
          25: { name: "NOT", args: 1, exec: (a) => a.not() },
          // BYTE (0x1a): extrait l'octet i du mot 256-bit x → stableSymbol.
          26: { name: "BYTE", args: 2, exec: (i, x) => stableSymbol("byte", i, x) },
          27: { name: "SHL", args: 2, exec: (shift, value) => value.shl(shift) },
          28: { name: "SHR", args: 2, exec: (shift, value) => value.lshr(shift) },
          ...
          29: { name: "SAR", args: 2, exec: (shift, value) => value.shr(shift) },
          // ── Hash ───────────────────────────────────────────────────────────────
          // keccak256 est une fonction de hachage cryptographique → non inversible par Z3.
          // Même couple (offset, length) → même symbole stable.
          32: { name: "SHA3", args: 2, exec: (offset, length) => stableSymbol("keccak256", offset, length) }
        };
      }
  ...
  var require_signatures = __commonJS({
    "src/signatures.js"(exports, module) {
      ...
      var LOCAL_SIGNATURES = {
        ...
        "0x3eaaf86b": "totalSupply()",
        // Some contracts use this alternative
        "0x91d14854": "admin()",
  ...
  var require_disassembler = __commonJS({
    "src/disassembler.js"(exports, module) {
      var MNEMONICS = {
        0: "STOP",
        1: "ADD",
        2: "MUL",
      ...
      function getMnemonic(opcode) {
        ...
        if (opcode >= 96 && opcode <= 127) return `PUSH${opcode - 96 + 1}`;
        if (opcode >= 128 && opcode <= 143) return `DUP${opcode - 128 + 1}`;
        if (opcode >= 144 && opcode <= 159) return `SWAP${opcode - 144 + 1}`;
        if (opcode >= 160 && opcode <= 164) return `LOG${opcode - 160}`;
      ...
      var Disassembler = class {
        ...
        static buildBasicBlocks(bytecode, validJumpDests, execLength) {
          ...
          while (i < execLength) {
            ...
            let data = null;
            if (opcode === 91 && validJumpDests.has(pc) && currentBlock.instructions.length > 0) {
              blocks.push(currentBlock);
              currentBlock = { startPc: pc, instructions: [] };
            }
            if (opcode >= 96 && opcode <= 127) {
              const pushSize = opcode - 96 + 1;
            ...
            i += 1;
            if ([86, 87, 0, 243, 253, 254].includes(opcode)) {
              blocks.push(currentBlock);
              if (i < execLength) {
        ...
        static async resolveSignatures(blocks) {
          const { resolveSignature } = require_signatures();
          for (const block of blocks) {
            for (let i = 0; i < block.instructions.length; i++) {
              const inst = block.instructions[i];
              if (inst.opcode === 99 && inst.data) {
                const nextInst = block.instructions[i + 1];
                const nextNextInst = block.instructions[i + 2];
                let isSelector = false;
                if (nextInst && nextInst.opcode === 20) isSelector = true;
                if (nextInst && nextInst.mnemonic.startsWith("DUP") && nextNextInst && nextNextInst.opcode === 20) isSelector = true;
                if (isSelector) {
  ...
  // src/stack_effects.js
  var require_stack_effects = __commonJS({
    "src/stack_effects.js"(exports, module) {
      var STACK_EFFECTS = {
        // Math & Logic
        0: [0, 0],
        1: [2, 1],
      ...
      function getStackEffect(opcode) {
        ...
        if (opcode >= 96 && opcode <= 127) return [0, 1];
        if (opcode >= 128 && opcode <= 143) return [opcode - 128 + 1, opcode - 128 + 2];
        return [0, 0];
      }
  ...
  var require_engine = __commonJS({
    "src/engine.js"(exports, module) {
      var { initZ3, SymbolicState } = require_state();
      var { getMathOpcodes } = require_opcodes();
      var { Disassembler, getMnemonic } = require_disassembler();
      var { getStackEffect } = require_stack_effects();
      var SymbolicEngine = class {
        constructor(bytecodeHex, z3Context, maxDepth = 1e3, txDepth = 1) {
          this.z3 = z3Context;
        ...
        _getExecutableLength() {
          ...
          if (metadataLen > 0 && metadataLen < len - 2) {
            ...
            if (marker === 161 || marker === 162 || marker === 163) {
              return cborStart;
            }
          }
          return len;
        ...
        async run(targetPc = null) {
          ...
          this.visitedStates = /* @__PURE__ */ new Set();
          let iter = 0;
        ...
        async executeOpcode(state, opcode) {
          if (opcode >= 95 && opcode <= 127) {
            let dataHex = "0";
            let pushSize = 0;
            if (opcode !== 95) {
              pushSize = opcode - 96 + 1;
          ...
          if (opcode === 0 || opcode === 243 || opcode === 253 || opcode === 254 || opcode === 255) {
            if (opcode === 253 || opcode === 254) return;
            if (state.txIndex < this.TX_DEPTH - 1) {
              ...
              nextState.txHistory = state.txHistory;
              nextState.pc = 0;
              nextState.depth = 0;
          ...
          if (opcode === 87) {
            ...
            if (["sat", "unknown"].includes(await this.solver.check())) {
              ...
              if (destPc !== null && this.validJumpDests.has(destPc)) {
                ...
                stateTrue.pc = destPc;
                stateTrue.pathConstraints.push(condTrue);
            ...
            if (["sat", "unknown"].includes(await this.solver.check())) {
              ...
              stateFalse.pathConstraints.push(condFalse);
          ...
          if (opcode === 86) {
            ...
            if (["sat", "unknown"].includes(await this.solver.check())) {
              ...
              if (model) {
                ...
                try {
                  firstDestPc = Number(firstConcrete.value());
                } catch (e) {
                }
                if (firstDestPc !== null) {
                  this.solver.push();
          ...
          if (state.stack.length < pops) return;
        ...
        async generatePoC(state) {
          ...
          const poc = {
            ...
            path_constraints: state.pathConstraints.map(
